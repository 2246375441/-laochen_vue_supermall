<template>
  <div>
    ...
    <div class="content" style="margin-top:50px">
      <div class="login-img">
        <div class="owl-login" :class="hideEyes?'password':''">
          <div class="hand"></div>
        ...
        <div class="core">
          <div class="core-login" v-show='isDOM' >
            <div>
              ...
              <input type="text" name="account" placeholder="请输入账号" v-model="login.account" onkeyup="value=value.replace(/[\u4e00-\u9fa5]/ig,'')" style="background:none">
            </div>
            <div>
              <span><i class="iconfont icon-mima" />密码</span>
              <input type="password" name="password" placeholder="请输入密码" v-model="login.password" onkeyup="value=value.replace(/[\u4e00-\u9fa5]/ig,'')" style="background:none" @focus="passwordF_B" @blur="passwordF_B">
            </div>
            <button class="loginBtn" @click="login_yzm_kq" style="background-color:none">登录</button>
          </div>

          <div class="core-login" v-show='!isDOM' >
            <div>
              <span><i class="iconfont icon-banzumingcheng" />名称</span>
              <input type="text" name="username" placeholder="请输入名称" v-model="register.username" style="background:none">
            </div>
            <div>
              <span><i class="iconfont icon-zhanghaodenglu" />账号</span>
              <input type="text" name="account" placeholder="请输入账号" v-model="register.account" onkeyup="value=value.replace(/[\u4e00-\u9fa5]/ig,'')" style="background:none">
            </div>
            <div>
              <span><i class="iconfont icon-mima" />密码</span>
              <input type="password" name="password" placeholder="请输入密码" v-model="register.password"  onkeyup="value=value.replace(/[\u4e00-\u9fa5]/ig,'')" style="background:none" @focus="passwordF_B" @blur="passwordF_B">
            </div>
            <button class="loginBtn" @click="register_yzm_kq" style="background-color:none">注册</button>
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted(){
    let btDom = this.$refs.contentBt
    let l = btDom.querySelector('.i-l').offsetLeft
    let r = btDom.querySelector('.i-r').offsetLeft
    let lr_box = this.$refs.lr_box
    let ll = lr_box.offsetLeft
    this.btLeft[0] = l - ll
    this.btLeft[1] = r - ll

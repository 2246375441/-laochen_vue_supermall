<template>
  <div class="box">
    ...
    <navbar class="navbar">
      ...
      <div slot="center">客服:{{kui.split('_')[0]}} (房间号:{{kui.split('_')[2]}})</div>
    </navbar>
    ...
    <div class="charlist" ref="charlistBS">
      <div>
        ...
        <!-- 聊天信息 -->
        <div v-for="(item) in charData" :class="item.fason==1?'item1':'item2'" :key="item.id">
          <div class="item1_l" v-show="item.fason==1">
          ...
          <div class="item1_r" v-show="item.fason==1">
            <img :src="item.img==''||item.img==null?'':tppj(item.img)" alt="">
          </div>


          <div class="item2_r" v-show="item.fason==0">
            <img :src="item.img==''||item.img==null?'':tppj(item.img)" alt="">
          </div>
          <div class="item2_l" v-show="item.fason==0">
              <div v-if="!item.content==''" class="wz_content">{{item.content}}</div>
              <!-- <img :src="tppj(item.content_img)" alt="" v-if="item.content==''" style="width:100%;height:auto;" :onerror="defaultImg"> -->
              <vant-image
                width="100%"
